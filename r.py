...
uri = "mongodb+srv://palaniappan0112:19930511@cluster0.12nzn.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
def clean_airbnb(df):
    if 'clean' not in st.session_state.keys():
        ...
        df.drop(columns=['listing_url', 'neighborhood_overview', 'cancellation_policy', 'summary', 'space', 'notes',
                         'description', 'images', 'first_review', 'last_scraped'], inplace=True)
        ...
        df.drop(columns=['address', 'availability', 'reviews', 'host', 'house_rules', 'interaction', 'review_scores',
                         'transit', 'access'], inplace=True)
        st.session_state['clean'] = 'Y'
        return df


if 'clean' not in st.session_state.keys():
    df_air = pd.read_json('sample_airbnb.json')
    st.session_state['df_map'] = clean_airbnb(df_air)

st.title("AIRBNB DATA ANALYSIS")
tab1, tab2, tab3 = st.tabs(
    [":clipboard: AIRBNB INTRO", ":clipboard: EDA", ":chart: DYNAMIC CHARTS"])
with tab1:
...
with tab2:
    ...
    if rb == "1. Countries with highest number of listings":
        ...
        pie_chart = pt.pie(df_f2, values='listing_id', title="Room types and listing count", names='room_type',
                           hover_data=['room_type'], height=600, width=600)
        ...
        line_chart = pt.line(out1, x="hostname", y="price", color='country', title='TOP 10 HOST WITH HIGHER RENTS',
        ...
        line_chart = pt.line(out1, x="hostname", y="price", color='country', title='TOP 10 HOST WITH LESSER RENTS',
        ...
        fig = pt.scatter(df_f3, x="room_type", y="no_of_amenities", size="no_of_amenities", color="country",
                         size_max=60, height=600, width=800)
        ...
        df_avail = df_map.sort_values(by=['availability_365', 'availability_30'], ascending=[True, True])[
            ['availability_365', 'availability_30', 'country', 'room_type', 'hostname']].head(10)
        df_avail.reset_index(inplace=True)
        df_f5 = df_avail[['country', 'room_type', 'hostname', 'availability_30', 'availability_365']]
        fig1 = pt.sunburst(df_f5, path=['country', 'room_type', 'availability_30', 'hostname'],
                           hover_data='availability_365', color='country')
        st.plotly_chart(fig1, theme=None)
    elif rb == "7. Listings having higher number of reviews":
        df_new = df_map[['country', 'number_of_reviews', 'listing_name', 'listing_id', 'rating']].sort_values(
            by='number_of_reviews', ascending=False).head(10)
        bar_chart1 = pt.bar(df_new, x='listing_name', y='number_of_reviews', hover_data=['country', 'rating'],
                            color='rating', title="Listing with high reviews", height=600, width=600)
...
with tab3:
    ...
    if option == "1.AVERAGE PRICE OF DIFFERENT ROOM TYPES":
        ...
        fig_bar = pt.bar(df_bar, x='room_type', y="price", title="Average Price for Different room types", width=800,
                         height=500, color='room_type')
    ...
    if option == "2.AVERAGE PRICE OF DIFFERENT PROPERTY TYPES BASED ON LOCATION":
        ...
        df_bar.reset_index(inplace=True)
        fig_bar = pt.bar(df_bar, x='property_type', y="price", title="Average Price for Different property types",
                         width=800, height=700, color="property_type")
    ...
    if option == "3.BASED ON AMENITIES":
        ...
        fig_bar = pt.bar(df_amen1, x='no_of_amenities', y="price", title="", hover_data=["price"], width=600,
                         height=500, color='price')
        st.plotly_chart(fig_bar, theme=None)

    if option == "4.MINIMUM NIGHTS BASED ON PROPERTY TYPE AND PRICE":
        min_night_option = st.radio("Select Minimum Nights:",
                                    ['<=5', '6 to 10', '11 to 20', '21 to 30', '31 to 40', '41 to 50', "\>50"])
